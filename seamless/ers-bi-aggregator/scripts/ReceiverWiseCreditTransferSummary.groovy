...
public class ReceiverWiseCreditTransferSummary extends AbstractAggregator {
    ...
    private SearchSourceBuilder buildESQuery(String fromDate, String toDate, Map<String, Object> afterKey) {
        ...
        sources.add(new TermsValuesSourceBuilder("transactionType").field("TransactionType.keyword").missingBucket(true));
        ...
        if (!bulkInsertionMode) {
            BoolQueryBuilder queryBuilder = QueryBuilders.boolQuery()
                    ...
                    .filter(QueryBuilders.termsQuery("transactionProfile.keyword",profileIdList))
    ...
    private Map<String, Object> generateResponse(SearchRequest searchRequest) {
        ...
        if (status == RestStatus.OK) {
            ...
            for (ParsedMultiBucketAggregation.ParsedBucket bucket : parsedComposite.getBuckets()) {
                ...
                String id = GenerateHash.createHashString(dateTimeDay.toString(), keyValuesMap.get("resellerId")
                        , keyValuesMap.get("resellerMSISDN")
                        ,keyValuesMap.get("resellerName")
                        , keyValuesMap.get("resellerType")
                        ,keyValuesMap.get("region")
                        ,keyValuesMap.get("transactionType")
                        ,keyValuesMap.get("receiverAccountType"));
                ReceiverWiseCreditTransferSummaryModel totalTransaction = new ReceiverWiseCreditTransferSummaryModel(id, dateTimeDay
                        , keyValuesMap.get("resellerId")
                        , keyValuesMap.get("resellerMSISDN")
                        ,keyValuesMap.get("resellerName")
                        , keyValuesMap.get("resellerType")
                        ,keyValuesMap.get("region")
                        , keyValuesMap.get("transactionType"),uniqueReceiverCount.value(), bucket.getDocCount()
