...
public class TransactionMaintainAggregator extends AbstractAggregator {
    ...
    private SearchSourceBuilder fetchInput(String fromDate, String toDate) {
        ...
        BoolQueryBuilder queryBuilder = QueryBuilders.boolQuery()
                .filter(QueryBuilders.termsQuery("NativeResultCode.keyword", "9001"))
        ...
        if (!bulkInsertionMode) {
            queryBuilder = queryBuilder.filter(QueryBuilders.rangeQuery("timestamp").gte("now" + timeOffset + "-24h/d").lt("now" + timeOffset + "+1h/d")
            ...
            if (status == RestStatus.OK) {
                ...
                for (SearchHit searchHit : searchHits.getHits()) {
                    Map<String, Object> searchHitMap = searchHit.getSourceAsMap();
                     ...
                     if(transactionProfile.startsWith("REVERSE")){
                     continue;
                      }
                    if(system=='CCPS' || system=='SKITTO'){

                    String reference_number =searchHitMap.get("ersReference")
                    String transaction_id =searchHitMap.get("transactionId")
                    ...
                    TransactionMaintainAggregatorModel transactionMaintainAggregatorModel = new TransactionMaintainAggregatorModel(reference_number,transaction_id,sender_id,sender_msisdn,receiver_msisdn,transaction_amount,currency,system,reconciliation_status,transaction_time,create_date,data,reconciliation_description)
    ...
    private def insertAggregation(List transactionMaintainAggregatorModelList) {
        ...
        if (transactionMaintainAggregatorModelList.size() != 0) {
            def sql = "INSERT INTO ${TABLE} (reference_number,transaction_id,sender_id,sender_msisdn,receiver_msisdn,transaction_amount,currency,system,reconciliation_status,transaction_time,create_date,data,reconcilliation_description" +
            ...
            def batchUpdate = jdbcTemplate.batchUpdate(sql, [
                    setValues   : { ps, i ->
                        ...
                        ps.setString(++index, row.transaction_id)
...
class TransactionMaintainAggregatorModel {
    ...
    TransactionMaintainAggregatorModel( String reference_number, String transaction_id, String sender_id, String sender_msisdn, String receiver_msisdn, Double transaction_amount, String currency, String system, String reconciliation_status, Date transaction_time, Date create_date, String data,String reconciliation_description) {
