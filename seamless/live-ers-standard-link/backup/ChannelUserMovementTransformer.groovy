...
public class ChannelUserBalanceEnquiryTransformer implements ERSIOTransformer
{
    ...
    public Message<?> transformInboundRequest(Message<?> message) throws TransformerException
    {
        ...
        try
        {
            ...
            else if(!command.get(Constants.Gp.LOGINID).toString().isEmpty())
            {
                dealerId.put(Constants.Txe.ID, command.get(Constants.Gp.LOGINID).toString());
                dealerId.put(Constants.Txe.TYPE, Constants.Txe.RESELLERID);
            }
            else
            {
                dealerId.put(Constants.Txe.ID, command.get(Constants.Gp.EXTCODE).toString());
                dealerId.put(Constants.Txe.TYPE, Constants.Txe.EXTCODE);
            }
        ...
        try
        {
            ...
            if (!internalResponse.isEmpty() && internalResponse.containsKey(StandardLinkConstants.Fields.RESULT_CODE))
            {
                ...
                if (resultCodeValue == StandardLinkResultCodes.SUCCESS)
                {
                    ...
                    internalResponse.put(Constants.Gp.GP_EXT_REQUEST_TYPE, operationFields.get(Constants.Gp.RESPONSE_TYPE));
                    SimpleDateFormat simpleDateFormat = new SimpleDateFormat(Constants.Gp.DATE_FORMAT);
                    internalResponse.put(Constants.Gp.GP_EXT_REQUEST_DATE, simpleDateFormat.format(new Date()));
                    ...
                    record.put(Constants.Gp.PRODUCTSHORTNAME, constants.get(Constants.PRODUCTSHORTNAME));
    ...
    private String fetchResellersBalance(Map<String, Object> responseMap)
    {
        ...
        return new DecimalFormat("0.00000").format(totalAvailableBalance);
    }
    private static final class Constants
    {
        ...
        {
            private static final String EXTCODE = "EXTERNALCODE";
            ...
            private static final String RESELLERID = "USERID";
        }
    }
}
