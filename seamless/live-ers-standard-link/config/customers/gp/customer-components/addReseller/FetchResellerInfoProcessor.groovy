...
@Component("fetchResellerInfoProcessor")
@ConditionalOnBean(name = "fetchResellerInfoInterceptor")
public class FetchResellerInfoProcessor implements InterceptedRequestProcessor
{
	private static final Logger LOGGER = LoggerFactory.getLogger(FetchResellerInfoProcessor.class);

	@Autowired
	@Qualifier("searchResellerByAttributeRequestChannel")
	private MessageChannel fetchResellerInfoChannel;
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("FetchResellerInfoProcessor :: getInfo");
		...
		try
		{
			...
			boolean send = fetchResellerInfoChannel.send(updatedMessage);
			...
			if (responseMessage.getPayload() instanceof LinkedCaseInsensitiveMap)
			{
				...
				if (responseMessageMap.containsKey(RESULT_CODE) && (Integer.valueOf(String.valueOf(responseMessageMap.get(RESULT_CODE))) == StandardLinkResultCodes.INTERNAL_SUCCESS || Integer.valueOf(String.valueOf(responseMessageMap.get(RESULT_CODE))) == StandardLinkResultCodes.SUCCESS))
				{
