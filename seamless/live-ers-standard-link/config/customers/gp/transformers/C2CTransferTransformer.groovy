...
@Component
public class C2CTransferTransformer implements ERSIOTransformer
{
	private static final Logger LOGGER = LoggerFactory.getLogger(C2CTransferTransformer.class);
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("C2CTransferTransformer :: transformOutboundRequest");
		...
		try
		{
			...
			return MessageBuilder.withPayload(transferRequest)
					...
					.setHeaderIfAbsent(Constants.Headers.SYSTEM_TOKEN, message.getHeaders().get(Constants.Headers.SYSTEM_TOKEN))
					.setHeaderIfAbsent(Constants.Headers.VALIDATE_ONLY, false)
					.build();
		}
		catch (Exception e)
		{
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("C2CTransferTransformer :: transformOutboundResponse");
	...
	private static final class Constants
	{
		...
		private static final String OPERATION = "c2cTransfer";
