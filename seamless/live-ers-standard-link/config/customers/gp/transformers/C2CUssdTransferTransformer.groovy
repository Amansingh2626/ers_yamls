...
@Component
public class C2CUssdTransferTransformer implements ERSIOTransformer
{

	private static final Logger LOGGER = LoggerFactory.getLogger(C2CUssdTransferTransformer.class);
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("C2CUssdTransferTransformer :: transformOutboundRequest");
		...
		try
		{
			...
			{
				sender.put(StandardLinkConstants.TXEConstant.ID, requestFields.get(Constants.Gp.MSISDN1).toString());
			...
			{
				receiver.put(StandardLinkConstants.TXEConstant.ID, requestFields.get(Constants.Gp.MSISDN2).toString());
			...
			return MessageBuilder.withPayload(transferRequest).copyHeaders(message.getHeaders()).setHeaderIfAbsent(StandardLinkConstants.Headers.SYSTEM_TOKEN, message.getHeaders().get("system-token")).setHeaderIfAbsent("validateOnly", false).setHeader("IS_REQUEST_TYPE_TEXT", requestFields.get("IS_REQUEST_TYPE_TEXT")).build();
		}
		catch (Exception e)
		{
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("C2CUssdTransferTransformer :: transformOutboundResponse");
	...
	private static final class Constants
	{
		...
		private static final String OPERATION = "c2cUSSDTransfer";
