...
@Component("c2SPromoRechargeTransformer")
public class C2SPromoRechargeTransformer implements ERSIOTransformer
{

	private static final Logger LOGGER = LoggerFactory.getLogger(C2SPromoRechargeTransformer.class);
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("C2SPromoRechargeTransformer :: transformOutboundRequest");
		...
		try
		{
			...
			{
				sender.put(StandardLinkConstants.TXEConstant.ID, command.get(Constants.Gp.MSISDN));
			...
			sender.put(StandardLinkConstants.TXEConstant.ACCOUNT_TYPE_ID, operationFields.get(Constants.Gp.SENDER_ACCOUNT_ID));

			if (!command.get(Constants.Gp.MSISDN2).toString().isEmpty())
			{
				receiver.put(StandardLinkConstants.TXEConstant.ID, command.get(Constants.Gp.MSISDN2));
			...
			map.put(Constants.Txe.SELECTOR, command.get(Constants.Gp.SELECTOR));
			...
			map.put(Constants.Gp.AMOUNT, command.get(Constants.Gp.AMOUNT));
			...
			return MessageBuilder.withPayload(promoRechargeRequest).copyHeaders(message.getHeaders()).setHeaderIfAbsent(Constants.SYSTEM_TOKEN, message.getHeaders().get(Constants.SYSTEM_TOKEN)).setHeaderIfAbsent(Constants.VALIDATE_ONLY, false).build();
		}
		catch (Exception e)
		{
	...
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("C2SPromoRechargeTransformer :: transformOutboundResponse");
		...
		try
		{
			...
			if (!internalResponse.isEmpty() && internalResponse.containsKey(Constants.Txe.RESULT_CODE))
			{
				...
				if (resultCodeValue == StandardLinkResultCodes.SUCCESS)
				{
					...
					response.put(Constants.Gp.TXNID, internalResponse.get(Constants.Txe.ERS_REFERENCE).toString());
					response.put(Constants.Gp.AMOUNT, requestPayloadCommand.get(Constants.Gp.AMOUNT).toString());
					response.put(Constants.Gp.TXNDATE, requestPayloadCommand.get(Constants.Gp.DATE).toString());
					response.put(Constants.Gp.SERVICETYPE, operationFields.get(Constants.Txe.SERVICE_TYPE));
				}
				else
				{
					ResultCode resultCode = standardLinkConfig.getResultCodeFor(Constants.PROVIDER, Constants.OPERATION, String.valueOf(resultCodeValue), String.valueOf(resultCodeValue), defaultMessage);
	...
	private static final class Constants
	{
		...
		private static final String OPERATION = "c2sPromoRecharge";
