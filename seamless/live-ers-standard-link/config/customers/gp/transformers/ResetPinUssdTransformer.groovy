...
@Component
public class ResetPinUssdTransformer implements ERSIOTransformer
{
	private static final Logger LOGGER = LoggerFactory.getLogger(ResetPinUssdTransformer.class);
		...
		try
		{
			...
			if (message.getHeaders().containsKey(Constants.SYSTEM_TOKEN))
			{
				...
				try
				{
					...
					if (systemTokenMap != null && systemTokenMap.size() > 0){
						Map<String, String> initiatorInfo = (Map) systemTokenMap.get(ResetPinUssdTransformer.Constants.Txe.CONTEXT).get(ResetPinUssdTransformer.Constants.Txe.INITIATOR);
						language = initiatorInfo.get(ResetPinUssdTransformer.Constants.Txe.LANGUAGE);
					}
				}
			...
			if (!newPin.equals(confirmPin))
			{
				return MessageBuilder.withPayload(new ChangeServicePassword())
						...
						.setHeader(ResetPinUssdTransformer.Constants.Gp.IS_REQUEST_TYPE_TEXT, requestFields.getOrDefault(ResetPinUssdTransformer.Constants.Gp.IS_REQUEST_TYPE_TEXT, "false"))
						.build();
			}
	...
	public Object transformInboundSoapResponse(Object outgoingResponse) throws TransformerException
	{
		...
		try
		{
			...
			LinkedCaseInsensitiveMap<Object> defaultFailMessage = (LinkedCaseInsensitiveMap<Object>) providerConfigurations.getOperations().get(ResetPinUssdTransformer.Constants.Gp.OPERATION).getFields().get(language);
			...
			if (returnMap!=null)
			{
				...
				ResultCode smsNotificationResultCode = standardLinkConfig.getResultCodeBasedOnLanguage(ResetPinUssdTransformer.Constants.Gp.PROVIDER, ResetPinUssdTransformer.Constants.Gp.OPERATION, String.valueOf(resultCodeValue), String.valueOf(resultCodeValue), defaultMessage, String.valueOf(language));
				...
				{
					LOGGER.info("Internal Result code is  {} " + resultCodeValue);
					smsNotificationMessage = smsNotificationMessage + " " + newPin;
				}
				else
				{
					LOGGER.debug("Failed result code is {} " + resultCodeValue);
					smsNotificationMessage = smsNotificationMessage + " " + smsReceiverMsisdn;
				}
...
//			smsNotificationUtility.sendSmsNotification("", String.valueOf(smsReceiverMsisdn), String.valueOf(smsNotificationMessage));
	...
	private static final class Constants
	{
		...
		private static final class Gp
		{
			...
			private static final String OPERATION = "resetPinUssd";
