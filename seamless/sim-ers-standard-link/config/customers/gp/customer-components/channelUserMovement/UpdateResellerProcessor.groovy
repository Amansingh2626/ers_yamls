...
@Component("updateResellerProcessor")
public class UpdateResellerProcessor implements InterceptedRequestProcessor
{
	private static final Logger LOGGER = LoggerFactory.getLogger(UpdateResellerProcessor.class);
	...
	@Autowired
	@Qualifier("updateResellerInternalRequestChannel")
	private MessageChannel updateResellerChannel;

	@Override
	public LinkedCaseInsensitiveMap<Object> process(Message<?> message)
	{
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("UpdateResellerProcessor :: update");
		...
		try
		{
			...
			boolean send = updateResellerChannel.send(updatedMessage);

			if (send)
			{
				LOGGER.info("UpdateReseller message successfully sent to entry channel");
			}
			else
			{
				LOGGER.error("UpdateReseller message could not be sent to entry channel");
			...
			if (responseMessage.getPayload() instanceof LinkedCaseInsensitiveMap)
			{
				...
				if ((responseMessageMap.containsKey(RESULT_CODE) && Integer.valueOf(String.valueOf(responseMessageMap.get(RESULT_CODE))) == StandardLinkResultCodes.SUCCESS)||(responseMessageMap.containsKey(RESULT_CODE) && Integer.valueOf(String.valueOf(responseMessageMap.get(RESULT_CODE))) == StandardLinkResultCodes.INTERNAL_SUCCESS))
				{
			...
			{
				LOGGER.error("An error occurred while processing UpdateReseller request");
