...
@Component
public class ViewChannelUserTransformer implements ERSIOTransformer
{

	private static final Logger LOGGER = LoggerFactory.getLogger(ViewChannelUserTransformer.class);
	...
	public Message<?> transformInboundRequest(Message<?> message) throws TransformerException
	{
		...
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("ViewChannelUserTransformer :: transformInboundRequest");
	...
	public Object transformOutboundResponse(Object outgoingResponse) throws TransformerException
	{
		...
		EtmPoint point = EtmManager.getEtmMonitor().createPoint("ViewChannelUserTransformer :: transformOutboundResponse");
		...
		try
		{
			...
			Map<String, Object> viewChannelUserResponse = new LinkedHashMap<>();
			...
			if (outgoingResponse instanceof Exception)
			{
				...
				viewChannelUserResponse.put(Constants.Gp.GP_MESSAGE, StandardLinkUtilities.getRootCause(exception));
				viewChannelUserResponse.put(Constants.Gp.GP_TXN_STATUS, StandardLinkResultCodes.INTERNAL_COMPONENT_ERROR);
			...
			viewChannelUserResponse.put(Constants.Gp.TYPE, operationFields.get(Constants.Gp.RESPONSE_TYPE));
			viewChannelUserResponse.put(Constants.Gp.DATE, simpleDateFormat.format(new Date()));
			...
			if (!responseMap.isEmpty() && responseMap.containsKey("resultCode"))
			{
				...
				viewChannelUserResponse.put(Constants.Gp.GP_TXN_STATUS, resultCode.getInternalResultCode());
			}
			else
			{
				resultCode = new ResultCode(String.valueOf(StandardLinkResultCodes.FAILED_ON_SERVER), "Request has failed on server.");
				viewChannelUserResponse.put(Constants.Gp.GP_MESSAGE, "Request has failed on server.");
				viewChannelUserResponse.put(Constants.Gp.GP_TXN_STATUS, StandardLinkResultCodes.FAILED_ON_SERVER);
				viewChannelUserResponse.put(Constants.Gp.ERRORKEY, responseMap.get(Constants.Gp.RESULTCODE).toString());
			}
			...
			{
				viewChannelUserResponse.put(Constants.Gp.EXTREFNUM, requestFields.get(Constants.Gp.EXTREFNUM).toString());
				...
				viewChannelUserResponse.put(Constants.Gp.DATA, data);
				Map<String, Object> balanceMap = accountMap != null ? (Map<String, Object>) accountMap.get(Constants.Dms.BALANCE) : null;
				if (balanceMap != null)
				{
					viewChannelUserResponse.put(Constants.Dms.RESELLER_BALANCE, balanceMap.getOrDefault(Constants.Dms.VALUE, 0));
				}
				else
				{
					viewChannelUserResponse.put(Constants.Dms.RESELLER_BALANCE, 0);
				}
			...
			{
				viewChannelUserResponse.put(Constants.Gp.GP_TXN_STATUS, resultCode.getInternalResultCode());
				viewChannelUserResponse.put(Constants.Gp.GP_MESSAGE, resultCode.getDescription());
			}
	...
	private boolean isCallForDeregisterInterceptor(Map<String, Object> additionalFields)
	{
		return Objects.nonNull(additionalFields.get(Constants.Gp.DATA)) &&
				Objects.nonNull(((LinkedHashMap<?, ?>) additionalFields.get(Constants.Gp.DATA)).get(Constants.Gp.USERORIGIN_ID)) &&
				StringUtils.isNotEmpty(((LinkedHashMap<?, ?>) additionalFields.get(Constants.Gp.DATA)).get(Constants.Gp.USERORIGIN_ID).toString());
