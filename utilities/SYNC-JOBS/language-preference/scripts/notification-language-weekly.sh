...
TEMP_FILE=${LOG_PATH}/source/weekly_reference_$(date +"%d-%m-%Y.csv.tmp");
LOG_FILE=${LOG_PATH}/log/weekly_reference_log_$(date +%d-%m-%Y).log;
FILENAME=weekly_reference_$(date +"%d-%m-%Y.csv");
...
if [ -e ${LOG_PATH}/source/weekly_reference_* ]
then
    mv ${LOG_PATH}/source/weekly_reference_* ${LOG_PATH}/backup/
...
logger INFO "Ready to download given file from remote server. FileName :  language_preference_dump_$(date +%d-%m-%Y).csv"

echo "ls ${REMOTE_PATH}/Language_Preference_FullDump_$(date +%d-%m-%Y).csv" | sshpass -p "${PASSWORD}" sftp -P ${PORT}  -oStrictHostKeyChecking=no  -b - ${USER}@${IP}


if [ $? -eq 0 ]
then
    logger INFO "Files exist, can download now"
    echo "get ${REMOTE_PATH}/Language_Preference_FullDump_$(date +%d-%m-%Y).csv ${LOG_PATH}/source/weekly_reference_$(date +%d-%m-%Y).csv.tmp" | sshpass -p "${PASSWORD}" sftp -P ${PORT}  -oStrictHostKeyChecking=no  -b - ${USER}@${IP}
...
then
  ...
  echo RESULT=$(curl --location --request POST "http://${NMS_HOST:-localhost}:${NMS_PORT:-8277}/register" --header 'Content-Type: application/json' --data '{
       ...
       "fields": {
           ...
           "message": "File '${REMOTE_PATH}/Language_Preference_FullDump_$(date +%d-%m-%Y).csv' not found or can'\''t reach Server",
...
logger INFO  "Notification preference weekly script started : $(date)"
move_older
#logger INFO "Ready to download given file from remote server. FileName : ${FILENAME}"
sftp_files
logger INFO "Notification preference weekly script ended :  $(date)"
